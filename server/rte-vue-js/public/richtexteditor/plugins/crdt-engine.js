...
(() => {
  ...
  __export(index_exports, {
    ...
    attachCrdtBinding: () => attachCrdtBinding,
    attachUndoManager: () => attachUndoManager,
    captureSelection: () => captureSelection,
    classify: () => classify,
    htmlToYFragment: () => htmlToYFragment,
    renderFragmentToHost: () => renderFragmentToHost,
    renderYNodeToDom: () => renderYNodeToDom,
...
${err.toString()}`);
  ...
  var INLINE_MARK_TAGS = /* @__PURE__ */ new Set([
    "strong",
    "b",
    "em",
    "i",
    ...
    "var",
    "span",
    // generic; mark stores className / style
    "font",
    // legacy; preserved for round-trip with old content
    "a"
    // href stored as a mark attribute
  ]);
  ...
  function classify(tag) {
    ...
    if (LEAF_TAGS.has(t)) return "leaf";
    if (INLINE_MARK_TAGS.has(t)) return "mark";
    return "unknown";
  }
  var SUPPORTED_MARKS = [
    "bold",
  ...
  function renderFragmentToHost(fragment, host) {
    ...
    const doc2 = host.ownerDocument;
    if (!doc2) {
      throw new Error("renderFragmentToHost: editable host has no ownerDocument");
    }
    for (let i = 0; i < fragment.length; i++) {
      const child = fragment.get(i);
      const node = renderYNodeToDom(child, doc2);
      if (node) host.appendChild(node);
    }
    if (host.childNodes.length === 0) {
      host.appendChild(doc2.createElementNS(HTML_NS, "p"));
    }
  }
  ...
  function renderYElementToDom(yel, doc2) {
    const tag = yel.nodeName.toLowerCase();
    const kind = classify(tag);
    if (kind === "unknown") {
      const span = doc2.createElementNS(HTML_NS, "span");
      span.setAttribute("data-rte-unknown", tag);
      applyAttributes(yel, span);
      appendChildren(yel, span, doc2);
      return span;
    }
    const el = doc2.createElementNS(HTML_NS, tag);
    applyAttributes(yel, el);
    if (kind === "leaf") {
      return el;
    }
    appendChildren(yel, el, doc2);
    return el;
  }
  function appendChildren(yel, host, doc2) {
    const len = yel.length;
    for (let i = 0; i < len; i++) {
      const child = yel.get(i);
      const node = renderYNodeToDom(child, doc2);
      if (node) host.appendChild(node);
    }
  }
  function applyAttributes(yel, el) {
  ...
  function renderYTextToDom(ytext, doc2) {
    var _a;
  ...
  function wrapForMark(mark, attrs, doc2) {
    switch (mark) {
      ...
      case "italic":
        return doc2.createElementNS(HTML_NS, "em");
      case "underline":
        return doc2.createElementNS(HTML_NS, "u");
      case "strikethrough":
        return doc2.createElementNS(HTML_NS, "s");
      case "subscript":
        return doc2.createElementNS(HTML_NS, "sub");
      case "superscript":
        return doc2.createElementNS(HTML_NS, "sup");
      case "code":
        return doc2.createElementNS(HTML_NS, "code");
      case "link": {
        const a = doc2.createElementNS(HTML_NS, "a");
      ...
      case "color": {
        const span = doc2.createElementNS(HTML_NS, "span");
        span.setAttribute("style", `color:${attrs["color"]}`);
        return span;
      }
      case "backcolor": {
        const span = doc2.createElementNS(HTML_NS, "span");
        span.setAttribute("style", `background-color:${attrs["backcolor"]}`);
        return span;
      }
      case "fontFamily": {
        const span = doc2.createElementNS(HTML_NS, "span");
        span.setAttribute("style", `font-family:${attrs["fontFamily"]}`);
        return span;
      }
      case "fontSize": {
        const span = doc2.createElementNS(HTML_NS, "span");
        span.setAttribute("style", `font-size:${attrs["fontSize"]}`);
        return span;
      }
      default:
        return doc2.createElementNS(HTML_NS, "span");
    }
  }

  // src/selection.ts
  function captureSelection(host) {
    var _a;
    const win = (_a = host.ownerDocument) == null ? void 0 : _a.defaultView;
    ...
    return {
      insideHost: true,
      anchorPath: pathFromHost(host, range.startContainer),
      anchorOffset: range.startOffset,
      focusPath: pathFromHost(host, range.endContainer),
      focusOffset: range.endOffset
    };
  }
  function restoreSelection(host, captured) {
    if (!captured.insideHost) return;
    const doc2 = host.ownerDocument;
    const win = doc2 == null ? void 0 : doc2.defaultView;
    if (!doc2 || !win) return;
    const selection = win.getSelection();
    if (!selection) return;
    const anchor = resolvePath(host, captured.anchorPath, captured.anchorOffset);
    const focus = captured.anchorPath === captured.focusPath ? anchor : resolvePath(host, captured.focusPath, captured.focusOffset);
    try {
  ...
  function pathFromHost(host, target) {
    const path = [];
    let cursor = target;
  ...
  function resolvePath(host, path, offset) {
    let cursor = host;
    for (const idx of path) {
      const child = cursor.childNodes.item(idx);
      if (!child) {
        return { node: cursor, offset: 0 };
      }
      cursor = child;
    }
    const safeOffset = clampOffset(cursor, offset);
    return { node: cursor, offset: safeOffset };
  }
  function clampOffset(node, offset) {
    if (node.nodeType === 3) {
      const len2 = node.data.length;
      return Math.max(0, Math.min(offset, len2));
    }
  ...
  // src/observer.ts
  function startLocalObserver(opts) {
    var _a, _b;
    const win = (_a = opts.editable.ownerDocument) == null ? void 0 : _a.defaultView;
    if (!win) {
      return { disconnect: () => {
      }, drainPending: () => {
      } };
    ...
    const observer = new MO((mutations) => {
      var _a2;
      if (opts.isApplyingRemote()) return;
    ...
    observer.observe(opts.editable, {
      childList: true,
      subtree: true,
      characterData: true,
  ...
  function handleMutation(m, opts) {
    switch (m.type) {
      case "characterData":
        handleCharacterData(m, opts);
        break;
      case "childList":
        handleChildList(m, opts);
        break;
      case "attributes":
        handleAttributes(m, opts);
        break;
    }
  }
  function handleCharacterData(m, opts) {
  ...
  function handleChildList(m, opts) {
    if (m.target === opts.editable) {
      rebuildFragmentFromEditable(opts);
      return;
    }
    if (m.target.nodeType !== 1) return;
  ...
  function handleAttributes(m, opts) {
    if (m.target.nodeType !== 1) return;
    const el = m.target;
    const yEl = locateYElement(el, opts);
    if (!yEl) return;
    if (m.attributeName === null) return;
    const newVal = el.getAttribute(m.attributeName);
    if (newVal === null) {
      yEl.removeAttribute(m.attributeName);
    } else if (yEl.getAttribute(m.attributeName) !== newVal) {
      yEl.setAttribute(m.attributeName, newVal);
    }
  }
  function locateYNodeForText(textNode, opts) {
    const path = pathFromHost2(opts.editable, textNode);
  ...
  function locateYElement(domEl, opts) {
    const path = pathFromHost2(opts.editable, domEl);
    if (path.length === 0) return null;
    let cursor = opts.fragment;
    for (const idx of path) {
      const child = cursor.get(idx);
  ...
  var RESYNC_PENDING = /* @__PURE__ */ new WeakSet();
  function scheduleCoarseResync(opts) {
    if (RESYNC_PENDING.has(opts.fragment)) return;
    RESYNC_PENDING.add(opts.fragment);
    queueMicrotask(() => {
      var _a;
      RESYNC_PENDING.delete(opts.fragment);
      (_a = opts.fragment.doc) == null ? void 0 : _a.transact(() => rebuildFragmentFromEditable(opts), LOCAL_ORIGIN);
    });
  }
  function rebuildFragmentFromEditable(opts) {
    opts.fragment.delete(0, opts.fragment.length);
    const built = buildYNodesFromDom(opts.editable);
    if (built.children.length > 0) {
      opts.fragment.insert(0, built.children);
      applyDeferredMarks(built.marked);
    }
  }
  function buildYNodesFromDom(host) {
    const result = { children: [], marked: [] };
    for (let i = 0; i < host.childNodes.length; i++) {
      const child = host.childNodes.item(i);
      const built = buildYNodeFromDomChild(child);
      if (!built) continue;
      if (built.node) result.children.push(built.node);
      for (const plan of built.marked) result.marked.push(plan);
    }
    return result;
  }
  function buildYNodeFromDomChild(node) {
    ...
    if (kind === "leaf") return { node: yel, marked: [] };
    const folded = foldInlineRunsToYChildren(Array.from(el.childNodes));
    for (const child of folded.children) {
      yel.insert(yel.length, [child]);
    }
    return { node: yel, marked: folded.marked };
  }
  function newTextBuffer() {
    return { str: "", ranges: [] };
  }
  function foldInlineRunsToYChildren(children) {
    const result = { marked: [], children: [] };
    let buffer = null;
    const flushText = () => {
    ...
    const ensureBuffer = () => {
      if (!buffer) buffer = newTextBuffer();
      return buffer;
    };
    for (const child of children) {
      ...
      if (kind === "mark") {
        const buf = ensureBuffer();
        const marks = marksFromMarkElement(el);
        collectMarkedSubtree(el, buf, marks);
        continue;
      }
      flushText();
      if (kind === "leaf") {
        ...
        const built = buildYNodeFromDomChild(el);
        if (built && built.node) {
          result.children.push(built.node);
          for (const plan of built.marked) result.marked.push(plan);
        }
      }
    }
  ...
  function collectMarkedSubtree(el, buf, parentMarks) {
    for (const child of Array.from(el.childNodes)) {
      if (child.nodeType === 3) {
        const data = child.data;
        if (data.length === 0) continue;
        const start = buf.str.length;
        buf.str += data;
        const end = buf.str.length;
        if (Object.keys(parentMarks).length > 0) {
          buf.ranges.push({ start, end, marks: parentMarks });
        }
        continue;
      }
      if (child.nodeType !== 1) continue;
      const childEl = child;
      const childKind = classify(childEl.tagName.toLowerCase());
      if (childKind === "mark") {
        const merged = { ...parentMarks, ...marksFromMarkElement(childEl) };
        collectMarkedSubtree(childEl, buf, merged);
      }
    }
  }
  function applyDeferredMarks(plans) {
    for (const plan of plans) {
      for (const range of plan.ranges) {
        const len = range.end - range.start;
        if (len <= 0) continue;
        plan.text.format(range.start, len, range.marks);
      }
    }
  }
  var __exportForMigration = {
    buildYNodesFromDom,
    applyDeferredMarks
  };
  function marksFromMarkElement(el) {
    ...
    switch (tag) {
      ...
      case "span":
      case "font": {
        return marksFromInlineStyle(el);
      }
      default:
        return {};
    }
  }
  function marksFromInlineStyle(el) {
    var _a;
    const out = {};
    const style = (_a = el.getAttribute("style")) != null ? _a : "";
      ...
      dispose() {
        for (const cleanup of cleanups) {
          try {
            ...
            console.error("[crdt-engine] dispose cleanup error:", err);
          }
        }
        cleanups = [];
      }
  ...
  function subscribeAwarenessStub(awareness, initial) {
    if (!awareness) return () => {
    };
    if (initial) awareness.setLocalState(initial);
    return () => awareness.setLocalState(null);
  }

  // src/undo.ts
  function attachUndoManager(opts) {
    var _a;
    const trackedOrigins = /* @__PURE__ */ new Set([LOCAL_ORIGIN]);
    if (opts.extraTrackedOrigins) {
      for (const o of opts.extraTrackedOrigins) trackedOrigins.add(o);
    }
    return new UndoManager(opts.fragment, {
      trackedOrigins,
      captureTimeout: (_a = opts.captureTimeoutMs) != null ? _a : 500
    });
  }

  // src/awareness.ts
  var DEFAULT_THROTTLE_MS = 32;
  function attachAwareness(opts) {
    var _a, _b;
    const win = (_a = opts.editable.ownerDocument) == null ? void 0 : _a.defaultView;
    const doc2 = opts.editable.ownerDocument;
    const throttle = (_b = opts.throttleMs) != null ? _b : DEFAULT_THROTTLE_MS;
    let lastPublishedAnchor = null;
    let lastPublishedHead = null;
    ...
    const publish = () => {
      ...
      if (!opts.editable.contains(range.startContainer) || !opts.editable.contains(range.endContainer)) {
        opts.awareness.setLocalStateField("cursor", null);
        return;
      }
      const anchor = encodeRelativePosition2(opts.fragment, opts.editable, range.startContainer, range.startOffset);
      const head = sel.isCollapsed ? anchor : encodeRelativePosition2(opts.fragment, opts.editable, range.endContainer, range.endOffset);
      if (!anchor || !head) {
        opts.awareness.setLocalStateField("cursor", null);
        return;
      }
      if (lastPublishedAnchor && lastPublishedHead && uint8Equal(lastPublishedAnchor, anchor) && uint8Equal(lastPublishedHead, head)) {
        return;
      }
      lastPublishedAnchor = anchor;
      lastPublishedHead = head;
      const payload = {
      ...
      opts.awareness.setLocalState({
        ...(_a2 = opts.awareness.getStates().get(opts.awareness.clientID)) != null ? _a2 : {},
        ...payload,
        active: Date.now()
      });
    };
    ...
    return {
      dispose() {
        ...
        if (doc2) doc2.removeEventListener("selectionchange", onSelectionChange);
        opts.awareness.setLocalStateField("cursor", null);
      },
      publishNow() {
      ...
      snapshotPeers() {
        var _a2, _b2;
        const out = [];
        for (const [clientID, raw] of opts.awareness.getStates()) {
          if (clientID === opts.awareness.clientID) continue;
          const state = raw;
          if (!state.cursor) continue;
          const anchor = decodeRelativePosition2(opts.fragment, opts.editable, state.cursor.anchor.encoded);
          const head = state.cursor.head === state.cursor.anchor ? anchor : decodeRelativePosition2(opts.fragment, opts.editable, state.cursor.head.encoded);
          out.push({
            clientID,
            user: (_a2 = state.user) != null ? _a2 : {},
            anchor,
            head,
            isCollapsed: anchor !== null && head !== null && anchor.node === head.node && anchor.offset === head.offset,
            activeAt: (_b2 = state.active) != null ? _b2 : 0
          });
        }
        return out;
      }
  ...
  function encodeRelativePosition2(fragment, editable, domNode, domOffset) {
    const yLocation = domToYPosition(fragment, editable, domNode, domOffset);
    if (!yLocation) return null;
    try {
      const rel = createRelativePositionFromTypeIndex(yLocation.parent, yLocation.index);
      return encodeRelativePosition(rel);
    } catch (e) {
      return null;
    }
  }
  function decodeRelativePosition2(fragment, editable, encoded) {
    if (!fragment.doc) return null;
    try {
      const rel = decodeRelativePosition(encoded);
      const abs2 = createAbsolutePositionFromRelativePosition(rel, fragment.doc);
      if (!abs2) return null;
      return yPositionToDom(editable, abs2.type, abs2.index);
    } catch (e) {
      return null;
    }
  }
  function domToYPosition(fragment, editable, domNode, domOffset) {
    var _a;
    const blockPath = [];
    let charOffset = domOffset;
    let current = domNode;
    if (current.nodeType === 3) {
      while (current && current !== editable) {
        const ancestor = current.parentNode;
        if (!ancestor) break;
        let prior = 0;
        let walk = ancestor.firstChild;
        while (walk && walk !== current) {
          prior += textLengthOf(walk);
          walk = walk.nextSibling;
        }
        charOffset += prior;
        if (ancestor === editable || isBlockOrLeafCandidate(ancestor)) {
          current = ancestor;
          break;
        }
        current = ancestor;
      ...
      let sum = 0;
      for (let i = 0; i < domOffset && i < current.childNodes.length; i++) {
        sum += textLengthOf(current.childNodes.item(i));
      }
      charOffset = sum;
    }
    if (current === editable) {
      const idx = (_a = pathFromHost3(editable, domNode === editable ? editable : domNode).at(0)) != null ? _a : 0;
      return { parent: fragment, index: idx };
    }
    let pathBuilder = current;
    while (pathBuilder && pathBuilder !== editable) {
      const pbParent = pathBuilder.parentNode;
      if (!pbParent) return null;
      let idx = 0;
      let walk = pbParent.firstChild;
      while (walk && walk !== pathBuilder) {
        idx++;
        walk = walk.nextSibling;
      }
      blockPath.unshift(idx);
      pathBuilder = pbParent;
    }
    let yCursor = fragment;
    for (const idx of blockPath) {
      const child = yCursor.get(idx);
      if (!(child instanceof YXmlElement)) return null;
      yCursor = child;
    }
    const yt = yCursor.get(0);
    if (yt instanceof YXmlText) {
      return { parent: yt, index: charOffset };
    }
    return { parent: yCursor, index: 0 };
  }
  function yPositionToDom(editable, yType, yIndex) {
    const path = [];
    let cursor = yType;
    let isFirst = true;
    while (cursor) {
      const item = cursor._item;
      if (!item) break;
      const parentRef = item.parent;
      if (!parentRef) break;
      let idx = 0;
      let found = false;
      if (parentRef instanceof YXmlFragment || parentRef instanceof YXmlElement) {
        for (let i = 0; i < parentRef.length; i++) {
          if (parentRef.get(i) === cursor) {
            idx = i;
            found = true;
            break;
          }
      ...
      if (!found) break;
      if (!(isFirst && yType instanceof YXmlText)) {
        path.unshift(idx);
      }
      isFirst = false;
      cursor = parentRef;
    }
    let domCursor = editable;
    for (const idx of path) {
      const child = domCursor.childNodes.item(idx);
      if (!child) return null;
      domCursor = child;
    }
    const textPos = walkToCharOffset(domCursor, yIndex);
    return textPos;
  }
  function walkToCharOffset(blockEl, charOffset) {
    let remaining = charOffset;
    const stack = [];
    for (let i = blockEl.childNodes.length - 1; i >= 0; i--) {
      stack.push(blockEl.childNodes.item(i));
    }
  ...
  function textLengthOf(node) {
    ...
    let sum = 0;
    for (let i = 0; i < node.childNodes.length; i++) sum += textLengthOf(node.childNodes.item(i));
    return sum;
  }
  function isBlockOrLeafCandidate(node) {
  ...
  function pathFromHost3(host, target) {
    const path = [];
    let cursor = target;
    while (cursor && cursor !== host) {
      const parent = cursor.parentNode;
  ...
  // src/host.ts
  function attachCollab(options) {
    var _a, _b;
    const mode = (_a = options.textSync) != null ? _a : false;
    if (mode === "crdt") {
      return attachCrdtBinding(options);
    }
    if (mode === true) {
      if (!options.legacySnapshotAttach) {
        throw new Error(
          "[crdt-engine] textSync: true requires `legacySnapshotAttach` to be provided by the host. See M5.1 in the sprint README for the back-compat contract."
        );
      }
      return options.legacySnapshotAttach(options);
    }
    const fragment = options.ydoc.getXmlFragment((_b = options.fragmentName) != null ? _b : "default");
    return {
      ydoc: options.ydoc,
      fragment,
      dispose() {
  ...
  // src/migrate.ts
  function htmlToYFragment(opts) {
    var _a;
    const fragmentName = (_a = opts.fragmentName) != null ? _a : "default";
    const fragment = opts.ydoc.getXmlFragment(fragmentName);
    const sandbox = opts.parserDocument.createElement("div");
    sandbox.innerHTML = opts.html;
    let host = sandbox;
    const bodyChild = sandbox.querySelector("body");
    if (bodyChild) host = bodyChild;
    const built = __exportForMigration.buildYNodesFromDom(host);
    if (opts.replace !== false && fragment.length > 0) {
      fragment.delete(0, fragment.length);
    }
    if (built.children.length > 0) {
      fragment.insert(fragment.length, built.children);
      __exportForMigration.applyDeferredMarks(built.marked);
    }
    return fragment;
  }
