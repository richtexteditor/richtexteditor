...
function ValidateLicense() {
	...
	try {
		...
		editor.getDocument.lcparts = lcparts;
		...
		if (licensed_product_id == 20200202 || licensed_product_id == 80800808 || licensed_product_id == 1202071668)
			productmatch = true;
		...
		var lictype = parseInt(lcparts[6]);
		...
		switch (lictype) {
			...
			case 2:
				if (islocalhost)
					break;
				//if (!serverip) {
				//	xh.open("GET", scope.img.getAttribute("ResourceHandler") + "?type=serverip&_ver=" + new Date().getTime(), false);
				//	xh.send("");
				//	if (xh.status != 200)
				//		return scope.UploadError(task.uploadid, "Failed to load uploader license info!");
	...
	return ReportLicenseOK(parseInt(lcparts[6]));//defined in rte.js
}
...
function CheckLicenseOK() {
	...
	__licconsolewarn(msg);	//defined in rte.js
